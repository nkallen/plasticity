<%- func.isStaticMethod ? 'static' : '' %> <%- func.name %>(<%- include('params.d.ts', { params: func.params }) %>):<%_ _%>
...
<%- func.isStaticMethod ? 'static' : '' %> async <%- func.name %>_async(<%- include('params.d.ts', { params: func.params }) %>): Promise<<%_ _%>
<%_ if (func.returns.length === 0) { %>void
