...
export function DatabaseProxy<TBase extends GConstructor<{ db: DatabaseLike }>>(Base: TBase) {
    return class DatabaseProxy extends Base implements DatabaseLike {
        ...
        async replaceTemporaryItem(from: visual.Item, to: c3d.Item): Promise<TemporaryObject> {
            return this.db.replaceTemporaryItem(from, to);
        }
